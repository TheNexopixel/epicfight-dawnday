...
import net.dawn_day.gameasset.animation.optional.type.SelectiveExecutionAttackProxy;
import net.dawn_day.gameasset.animation.optional.type.SelectiveExecutionHitAnimation;
...
public class DawnDayExecAnims {
    ...
    private static final ExtraDamageInstance.ExtraDamage TARGET_MAX_HEALTH = new ExtraDamageInstance.ExtraDamage(
            (attacker, itemstack, target, baseDamage, params) -> params[0] + target.getMaxHealth() * params[1], (itemstack, tooltips, baseDamage, params, e) -> {
    });
    ...
    public static void build(AnimationManager.AnimationBuilder builder) {
        ...
        MILADY_EXECUTION = builder.nextAccessor("biped/execution/milady/milady_execution", (accessor)->
                milady(accessor,executionCollider,CONSTANT_EXECUTION));
        ...
        get2PhaseExecAtkAnim(accessor,executionCollider,CONSTANT_EXECUTION));
...
// Thats for Battlestaff
        ...
        return (new ExecutionAttackAnimation(0.01F, accessor,
                ...
                Armatures.BIPED, new ExecutionAttackAnimation.ExecutionPhase[]{(new ExecutionAttackAnimation.ExecutionPhase(false, 0.0F, 0.0F, (float) 0.8, (float) 0.85, 12.73F, 1.23F, Armatures.BIPED.get().rootJoint, executionCollider))
                .addProperty(AnimationProperty.AttackPhaseProperty.HIT_SOUND, EpicFightSounds.BLUNT_HIT.get())
                .addProperty(AnimationProperty.AttackPhaseProperty.DAMAGE_MODIFIER, ValueModifier.multiplier(0.2F)),

                (new ExecutionAttackAnimation.ExecutionPhase(true, 1.23F, 0.0F, (float) 1.42, (float) 1.45, 18.0F, 20.0F, Armatures.BIPED.get().rootJoint, executionCollider))
...
// Thats for Milady
        ...
        return (new ExecutionAttackAnimation(0.01F, accessor,
                ...
                Armatures.BIPED, new ExecutionAttackAnimation.ExecutionPhase[]{(new ExecutionAttackAnimation.ExecutionPhase(false, 0.0F, 0.0F, (float) 0.22, (float) 0.23, 12.73F, 1.23F, Armatures.BIPED.get().rootJoint, executionCollider))
                ...
                (new ExecutionAttackAnimation.ExecutionPhase(true, 1.23F, 0.0F, (float) 1.77, (float) 1.78, 18.0F, 20.0F, Armatures.BIPED.get().rootJoint, executionCollider))
